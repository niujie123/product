...
function getConfig(){
    if(isset($_GET['applicationName'])){
        setcookie('applicationName', $_GET['applicationName'], time()+86400*30);
        return $_GET['applicationName'];
    }elseif(isset($_COOKIE['applicationName'])){
        return $_COOKIE['applicationName'];
    }else{
        header("Content-type: text/html; charset=utf-8");
        die("您的登录已经失效,请重新打开首页登录!");
    }
}
define("APPLICATION_NAME", getenv("APPLICATION_NAME")?getenv("APPLICATION_NAME"):'vrAdmin');
	...
	if (move_uploaded_file($tmp_name, $file_path) === false) {
		alert("上传文件失败。");
            ...
            if(is_null($options['url'])){
                $file_url= $response_upload_file_by_file->header['_info']['url'];
            }else{
                $file_url= $options['url'].'/'.str_replace('.', '/', $options['path']).'/'.$fileName;
            }
        }
